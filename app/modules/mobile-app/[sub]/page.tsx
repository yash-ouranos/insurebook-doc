...
import { flattenSubModules, getModule, getSubModule } from '@/lib/modules'

export function generateStaticParams() {
  const mod = getModule('mobile-app')
  return flattenSubModules(mod?.subModules)
    .filter((s) => !s.documented && !s.isGroup)
    .map((s) => ({ sub: s.slug }))
}
...
export async function generateMetadata({
  ...
  const entry = getSubModule('mobile-app', sub)
  return {
    title: entry
...
export default async function PendingMobileAppSubPage({
  params,
}: {
  params: Promise<{ sub: string }>
}) {
  ...
  if (!entry || entry.documented || entry.isGroup) {
    notFound()
  }
      ...
      <div className="mt-10 flex flex-col items-center gap-4 rounded-lg border border-dashed border-border bg-card px-6 py-14 text-center">
        ...
        <div>
          ...
          <p className="mx-auto mt-1.5 max-w-md text-sm leading-relaxed text-muted-foreground text-pretty">
            This section has not been documented yet. Share a screenshot of
            the {entry.name} screen from the mobile app in the chat and its
